-- Paginated job_ids for a single location drill-down (Locations tab).
--
-- Same filter predicates as get_location_insights, plus a granularity-gated
-- equality predicate on metro or province code. The null bucket is selected
-- with an empty-string sentinel (codes are never empty). Bounded page plus
-- empty-page total sentinel, mirroring get_keyword_job_ids (0003).
...
create or replace function public.get_location_job_ids(
  ...
  p_granularity text default 'city', p_metro text default '', p_province text default '',
...
language sql stable security invoker set search_path = '' as $$
  with matching as (
    ...
    where j.is_active is true
      and (p_granularity <> 'city'
        or (p_metro = '' and j.location_metro is null)
        or (p_metro <> '' and j.location_metro = p_metro))
      and (p_granularity <> 'province'
        or (p_province = '' and j.location_province_code is null)
        or (p_province <> '' and j.location_province_code = p_province))
...
revoke all on function public.get_location_job_ids(text[],text[],text[],text,text[],text[],text[],text[],text[],text,text,text,integer,integer) from public, anon, authenticated;
grant execute on function public.get_location_job_ids(text[],text[],text[],text,text[],text[],text[],text[],text[],text,text,text,integer,integer) to service_role;
