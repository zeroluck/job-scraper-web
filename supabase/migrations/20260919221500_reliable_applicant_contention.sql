-- Replace source-bound applicant counts with a conservative pace based only on
-- exact observations. Growth needs a six-hour window; single observations are
-- normalized over at least 24 hours so fresh listings cannot create rate spikes.
...
AS $$
  SELECT entry ->> 'applicant_count' ~ '^\d+$'
    ...
    AND (
      entry ->> 'applicant_count_type' = 'exact'
      OR (
        entry ->> 'applicant_count_type' IS NULL
        AND (entry ->> 'applicant_count')::integer <> 25
      )
    )
$$;
...
AS $$
  ...
  SELECT
    ...
    public.listing_latest_applicant_count(to_jsonb(j.listing_instances)),
    public.listing_latest_applicant_observed_at(to_jsonb(j.listing_instances)),
    now()
  FROM public.jobs j
...
BEGIN
  INSERT INTO public.job_location_buckets (
    ...
    public.listing_latest_applicant_count(to_jsonb(NEW.listing_instances)),
    public.listing_latest_applicant_observed_at(to_jsonb(NEW.listing_instances)),
    now()
  )
  ON CONFLICT (job_id) DO UPDATE SET
    city_bucket = EXCLUDED.city_bucket,
    ...
    WITH membership_ids AS MATERIALIZED (
      ...
      SELECT s.*,
        CASE WHEN s.latest_applicant_count IS NOT NULL
            ...
            AND s.latest_applicant_observed_at >= s.first_applicant_observed_at + interval '6 hours'
        ...
        CASE WHEN s.latest_applicant_count IS NOT NULL
            AND s.latest_applicant_observed_at > s.effective_posted_at
          THEN s.latest_applicant_count::numeric /
            greatest(extract(epoch FROM (s.latest_applicant_observed_at - s.effective_posted_at)) / 3600, 24)
        END AS normalized_rate,
        CASE WHEN s.latest_applicant_observed_at > s.effective_posted_at
          THEN extract(epoch FROM (s.latest_applicant_observed_at - s.effective_posted_at)) / 3600
      ...
      SELECT s.bucket, max(s.city_label) AS city_label,
        ...
        count(coalesce(s.observed_rate, s.normalized_rate))::bigint AS contention_jobs,
        count(s.observed_rate)::bigint AS observed_contention_jobs,
        percentile_cont(0.5) WITHIN GROUP (ORDER BY coalesce(s.observed_rate, s.normalized_rate))
          FILTER (WHERE coalesce(s.observed_rate, s.normalized_rate) IS NOT NULL) AS applicants_per_hour,
        percentile_cont(0.5) WITHIN GROUP (ORDER BY s.latest_applicant_count)
          FILTER (WHERE s.latest_applicant_count IS NOT NULL) AS initial_applicants_median,
