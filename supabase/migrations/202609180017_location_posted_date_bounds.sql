-- Date-bounded location RPCs keep the canonical unbounded signatures intact
-- while allowing inclusive calendar-day filters without overload ambiguity.
...
CREATE FUNCTION public.get_location_job_ids_date_bounds(
  p_providers text[] DEFAULT NULL,
  p_archetypes text[] DEFAULT NULL,
  p_levels text[] DEFAULT NULL,
  p_filter_status text DEFAULT NULL,
...
BEGIN
  RETURN QUERY EXECUTE $query$
    WITH target AS MATERIALIZED (
      ...
      SELECT b.job_id FROM target t JOIN public.job_location_buckets b
        ON b.province_bucket = t.bucket OR (t.bucket LIKE '%|' AND b.province_bucket LIKE t.bucket || '%')
      WHERE $10 = 'province'
      UNION ALL
      SELECT b.job_id FROM target t JOIN public.job_location_buckets b
        ON b.folded_city_bucket = t.bucket OR (t.bucket LIKE '%|' AND b.folded_city_bucket LIKE t.bucket || '%')
      WHERE $10 <> 'province' AND $11
      UNION ALL
      SELECT b.job_id FROM target t JOIN public.job_location_buckets b
        ON b.city_bucket = t.bucket OR (t.bucket LIKE '%|' AND b.city_bucket LIKE t.bucket || '%')
      ...
      WHERE b.is_active AND ($2 IS NULL OR m.job_id IS NOT NULL)
        ...
        AND ($8 IS NULL OR $7 IS NOT NULL OR b.location_scope = ANY($8) OR $8 && b.listing_location_scopes)
        AND ($9 IS NULL OR b.location_metro IS NULL OR NOT (b.location_metro = ANY($9)))
        AND ($16 IS NULL OR b.effective_posted_at >= $16)
        AND ($17 IS NULL OR b.effective_posted_at < $17)
    ), ranked AS (
  ...
  $query$ USING
    ...
    p_posted_after, p_posted_before;
END;
$$;

REVOKE ALL ON FUNCTION public.get_location_insights_date_bounds(
  text[],text[],text[],text,text[],text[],text[],text[],text[],text,boolean,text,timestamptz,timestamptz
) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_location_insights_date_bounds(
  text[],text[],text[],text,text[],text[],text[],text[],text[],text,boolean,text,timestamptz,timestamptz
) TO service_role;
REVOKE ALL ON FUNCTION public.get_location_job_ids_date_bounds(
  text[],text[],text[],text,text[],text[],text[],text[],text[],text,boolean,text,text,integer,integer,timestamptz,timestamptz
) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_location_job_ids_date_bounds(
  text[],text[],text[],text,text[],text[],text[],text[],text[],text,boolean,text,text,integer,integer,timestamptz,timestamptz
) TO service_role;
